...
Product.Config.prototype.showCustomOptionsBlock = function(productId, parentId) {
   ...
   if ($('JTLcustomOptionsDiv')==null) {
      return;
   }

    Effect.Fade('JTLcustomOptionsDiv', { duration: 0.5, from: 1, to: 0.5 });
    if(productId) {
        //Uncomment the line below if you want an ajax loader to appear while any custom
        //options are being loaded.
        //$$('span.jtl-please-wait').each(function(el) {el.show()});

        //prodForm.getElements().each(function(el) {el.disable()});
        new Ajax.Updater('JTLcustomOptionsDiv', coUrl, {
          method: 'get',
          evalScripts: true,
          onComplete: function() {
              $$('span.jtl-please-wait').each(function(el) {el.hide()});
              Effect.Fade('JTLcustomOptionsDiv', { duration: 0.5, from: 0.5, to: 1 });
              //prodForm.getElements().each(function(el) {el.enable()});
          }
        });
    } else {
        $('JTLcustomOptionsDiv').innerHTML = '';
